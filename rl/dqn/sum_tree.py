...
class SumTree(object):
    ...
    def update(self, data_index, priority):
        ...
        tree_index = (
            data_index + self.buffer_capacity - 1
        )  # 把当前数据在buffer中的index转换为在sum_tree中的index
    ...
    def get_index(self, v):
        ...
        )  # 返回采样到的data在buffer中的index,以及相对应的priority
    ...
    def get_batch_index(self, current_size, batch_size, beta):
        ...
        segment = (
            self.priority_sum / batch_size
        )  # 把[0,priority_sum]等分成batch_size个区间，在每个区间均匀采样一个数
    ...
    def priority_max(self):
        ...
        ].max()  # 树的最后一层叶节点，保存的才是每个数据对应的priority
